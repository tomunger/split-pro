...
export const ImportExpensesFromCsv: React.FC<{
  ...
  const isIncluded = useCallback(
    (row: ParsedRow) =>
      !row.error && (overrides[row.lineNumber] ?? !duplicateLines.has(row.lineNumber)),
    [overrides, duplicateLines],
  );
  ...
  const onImport = useCallback(async () => {
    ...
    const expenses = selectedRows.flatMap((row) =>
      row.date ? [toExpense(row, row.date, paidBy)] : [],
    );
    ...
    try {
      for (const batch of chunk(expenses, IMPORT_BATCH_SIZE)) {
        await addExpenseMutation.mutateAsync(batch);
    ...
    <>
      ...
      <div className="mt-3 flex flex-col gap-3">
        {rows.map((row) => (
          <PreviewRow
            ...
            isDuplicate={duplicateLines.has(row.lineNumber)}
      ...
      <div className="bg-background sticky bottom-0 mt-6 flex justify-center py-4">
        <Button
          ...
          disabled={isImporting || 0 === selectedRows.length}
          onClick={onImport}
        >
          {isImporting ? (
            <>
...
}> = ({ row, included, isDuplicate, amount, onToggle }) => {
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <p className="truncate">{row.name || t('group_details.import_csv.no_name')}</p>
